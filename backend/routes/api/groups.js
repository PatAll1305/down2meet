...
router.get('/', async (req, res) => {
    const errors = {}
    const where = {}
    const { name, type, private, city, state, numMembers } = req.params
    if (name) {
        where.name = name
    }
    if (type) {
    ...
    for (let group of groups) {
        ...
        groupA.numMembers = await Membership.count({
            where: {
                groupId: groupA.id,
        ...
        if (image.id) {
            groupA.previewImage = image.url;
        } else {
            groupA.previewImage = image;
        }
        results.push(groupA);
    }
